...
export function Settings() {
  ...
  const handleImgViewer = (e: T) => {
    const file = e.target.files[0],
      fileType = file.type,
      validateImgTypes = ["image/jpg", "image/jpeg", "image/png"];

    if (!validateImgTypes.includes(fileType)) {
      sweetErrorHandling(Messages.error5);
    } else {
      if (file) memberUpdateInput.memberImage = file;
      setMemberUpdateInput({ ...memberUpdateInput });
  ...
  return (
    <Box className={"settings"}>
      <Box className={"member-media-frame"}>
        <img src={img} className={"mb-image"} />
        <div className={"media-change-box"}>
          <span>Upload image</span>
          <p>JPG, JPEG, PNG formats only!</p>
          <div className={"up-del-box"}>
            <Button component="label" onChange={handleImgViewer}>
              <CloudDownloadIcon />
